    ...
    try {
      ...
      if (req.userId.toString() !== currentComment.authorId.toString()) {
        return res.status(403).json({
          success: false,
          message: "unauthorised to delete comment, forbidden",
        });
      }
      ...
      check 3 possibilities:
        isReply ->
          post.totalComments - 1
          parent comment .replies .remove (currentComment)
          delete
        comment with 0 reply ->
          post.comments.remove (current comment id), post.totalComments - 1
          Delete
        comment with 1+ replies ->
      ...
      */
      ...
      if (currentComment.isReply) {
        ...
        await Comment.findByIdAndUpdate(currentComment.parentId, {
          $pull: { replies: currentComment._id },
        });

        await Comment.findByIdAndDelete(currentComment._id);
      } else if (
        ...
        await Comment.findByIdAndDelete(currentComment._id);
      } else if (!currentComment.isReply && currentComment.replies.length > 0) {
        post.totalComments -= 1;
