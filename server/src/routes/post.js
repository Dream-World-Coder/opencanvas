        ...
        if (existing) {
            if (existing.type === vote) {
                ...
                if (vote === "like") {
                    await mongoose
                        .model("User")
                        .findByIdAndUpdate(post.authorId, {
...
router.post("/post/save-unsave", authenticateToken, async (req, res) => {
    try {
        ...
        if (existing) {
            ...
            return res
                .status(200)
                .json({ success: true, message: "Post unsaved" });
        }
        ...
        const [saves, total] = await Promise.all([
            Interaction.find({
                ...
                .populate({
                    path: "targetId",
                    model: "Post",
                    select: "title contentPreview slug type tags readTime thumbnailUrl isPremium isPublic authorSnapshot stats createdAt",
                }),
