...
import { Search, Loader2, FileText, Library } from "lucide-react";
...
// ::::: Search Results Page :::::
//
// Route: /search?q=...&type=users|posts|publications|all
//
// Fetches from GET /search on mount and whenever the query changes.
// Tab switching (All / Users / Posts / Publications) is purely client-side - no extra requests.
...
export default function SearchPage() {
    ...
    // { users: [], posts: [], publications: [] }
    const [results, setResults] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    // "all" | "users" | "posts" | "publications"
    ...
    const fetchResults = async (q) => {
        ...
        try {
            ...
            setResults(res.data.data); // { users, posts, publications }
        } catch (err) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-white dark:bg-[#0f0f0f] pt-24 px-4 py-10 max-w-2xl mx-auto">
                ...
                >
                    <div className="relative flex-1">
                        ...
                        <Input
                            ...
                            placeholder="Search people, posts, or publications..."
                ...
                {/* ── Tabs ──────────────────────────────────────────────────────────── */}
                {results && (
                    <div className="flex gap-1 mb-6 border-b border-gray-100 dark:border-[#222] overflow-x-auto scrollbar-hide">
                        {tabs.map((tab) => (
                            <button
                                key={tab.key}
                ...
                {!query && !loading && (
                    <p className="text-center text-gray-400 py-16 text-sm">
                        Type something to search for people, posts, or
                        publications.
                    </p>
                )}
...
function PublicationSearchCard({ pub }) {
    // Show top 2 authors to save space in compact view
    const authorsDisplay = pub.authors
        ?.slice(0, 2)
        .map((a) => a.name)
        .join(", ");
    const hasMoreAuthors = pub.authors?.length > 2;

    return (
        <li>
            <Link
                to={`/publications/${pub.institution}/${pub._id}`}
                className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors"
            >
                <div className="w-14 h-14 rounded-md bg-gray-100 dark:bg-[#222] flex items-center justify-center flex-shrink-0">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">
                        ...
                        <span className="uppercase">{pub.institution}</span>
                    </p>
                </div>
            </Link>
        </li>
...
PublicationSearchCard.propTypes = {
